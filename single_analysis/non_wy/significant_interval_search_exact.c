#ifndef _significant_interval_search_exact_c_
#define _significant_interval_search_exact_c_
...
int main(int argc, char *argv[]){
	...
	if(argc < 6){
		...
		printf("\tUSAGE: ./program_name X_file Y_file alpha L_max base_filename [out_file_pvals]\n");
		exit(1);
	}
	...
	tmp_filename = (char *)malloc((strlen(argv[5])+50)*sizeof(char));


	// Create a file to report significant intervals
	strcpy(tmp_filename,argv[5]); strcat(tmp_filename,"_sigints.csv");
	...
	strcpy(tmp_filename,argv[5]); strcat(tmp_filename,"_timing.txt");
	...
	strcpy(tmp_filename,argv[5]); strcat(tmp_filename,"_summary.txt");
	...
	if(argc==7){
		...
		if(!(pval_out_file = fopen(argv[6],"w"))){
			fprintf(stderr, "Error in function main when opening file %s\n",argv[6]);
			exit(1);
		}
		// If the file was successfully create, write the file header
	...
	sis_init(argv[1], argv[2], atof(argv[3]), atoll(argv[4]));
	...
	//R_command = (char *)malloc((strlen(argv[5])+100)*sizeof(char));
	strcpy(R_command,"python pyfiltering.py "); strcat(R_command,argv[5]);
	strcat(R_command,"_sigints.csv "); strcat(R_command,argv[5]); strcat(R_command,"_sigints_filtered.corrected.csv");
