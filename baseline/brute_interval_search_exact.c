...
void process_first_layer_pvalues(){
	...
	for(tau=0; tau<L; tau++){
		...
		// Compute the p-value
		pval = fisher_pval(a,freq_par[tau]);
...
int main(int argc, char *argv[]){
	...
	strcpy(R_command,"Rscript rfiltering.R "); strcat(R_command,argv[5]);
	strcat(R_command,"_sigints.csv "); strcat(R_command,argv[5]); strcat(R_command,"_sigints_filtered.csv");
	system(R_command);
	free(R_command);
